...
class InfluxObject(object):
    ...
    def connect_influx_db(self, db_name):
        ...
        """
	protocol = ""
	protocolPosition = self.db_server_name.rfind("//");
	if protocolPosition != -1:
	    protocol = self.db_server_name[0:self.db_server_name.rfind("//") + 2]
	    host = self.db_server_name[self.db_server_name.rfind("//")+2:self.db_server_name.rfind(':')]
	else:
	...
	usessl = False
		...
		usessl = True
                ...
                port = 80
		usessl = False
        client = InfluxDBClient(host, port, self.db_user, self.db_password, db_name, ssl=usessl, timeout=120)
